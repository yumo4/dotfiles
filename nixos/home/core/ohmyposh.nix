...
# }
{
  ...
  programs.oh-my-posh = {
    ...
    settings = {
      ...
      # Add right-side transient prompt for SSH indicator
      rtransient_prompt = {
        template = "{{ if .Env.SSH_CONNECTION }}<magenta>ssh:</><cyan>{{ .Env.SSH_CLIENT | regexFind \"^[^\\s]+\" }}</>{{ end }}";
        background = "transparent";
        foreground = "cyan";
      };
      ...
      blocks = [
        ...
        # Add right-aligned block for SSH indicator in normal prompt
        {
          type = "prompt";
          alignment = "right";
          segments = [
